...
export const assignParcelClusterToRider = async (parcelListId, riderId, organizationId = null, options = {}) => {
  try {
    ...
    const findNearestLoadableStopIndex = (startLat, startLng, stops, capacityKg, currentLoadKg) => {
      let fallbackIndex = 0;
      ...
      stops.forEach((stop, index) => {
        const distance =
          ...
          isFiniteCoordinate(stop.longitude)
            ? haversineKm(startLat, startLng, stop.latitude, stop.longitude)
            : index;
        const weightKg = toFiniteNumber(stop.weightKg);
        const fitsCurrentLoad = capacityKg <= 0 || currentLoadKg + weightKg <= capacityKg;
    ...
    const buildLoadAwareStops = ({ startLat, startLng, stops, capacityKg, serviceArea }) => {
      const safeCapacityKg = Number.isFinite(Number(capacityKg)) ? Math.max(0, Number(capacityKg)) : 0;
      const hasServiceArea =
        serviceArea &&
        isFiniteCoordinate(serviceArea.lat) &&
        isFiniteCoordinate(serviceArea.lng);

      if (!hasServiceArea || safeCapacityKg <= 0) {
        return orderStopsByNearestNeighbor(startLat, startLng, stops).map((stop, index) => ({
          ...stop,
          rideNumber: 1,
          loadWeightKg: toFiniteNumber(stop.weightKg),
      ...
      while (remaining.length > 0) {
        const nextIndex = findNearestLoadableStopIndex(
          currentLat,
          currentLng,
          remaining,
          safeCapacityKg,
        ...
        const nextStop = remaining[nextIndex];
        const nextWeightKg = toFiniteNumber(nextStop.weightKg);
        const wouldExceedCapacity =
          currentLoadKg > 0 && currentLoadKg + nextWeightKg > safeCapacityKg;

        if (wouldExceedCapacity) {
          plannedStops.push({
            parcelId: null,
            parcelListId: null,
            shipmentTrackingId: `SERVICE-AREA-RETURN-${rideNumber}`,
            fallbackSequence: plannedStops.length + 1,
            latitude: serviceArea.lat,
            longitude: serviceArea.lng,
            address: serviceArea.name || 'Service Area',
            weightKg: 0,
            isServiceAreaStop: true,
            rideNumber,
            loadWeightKg: currentLoadKg,
          });
    ...
    const optionServiceArea = options?.serviceAreaPoint || null;
    const fallbackServiceArea =
      isFiniteCoordinate(riderStartLat) && isFiniteCoordinate(riderStartLng)
        ? {
            name: 'Service Area',
            lat: riderStartLat,
            lng: riderStartLng,
        ...
        : null;
    const serviceArea = optionServiceArea || fallbackServiceArea;
    const orderedStops = buildLoadAwareStops({
      startLat: riderStartLat,
      startLng: riderStartLng,
    ...
    const deliveryStopsPayload = orderedStops.map((stop, index) => ({
      ...
      parcel_id: stop.isServiceAreaStop ? null : stop.parcelId || null,
      parcel_list_id: stop.isServiceAreaStop ? null : stop.parcelListId || null,
